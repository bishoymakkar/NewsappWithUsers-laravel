...
import NewsList from "./dbSport.json";
...
class NewsSport extends React.Component {
  ...
  render() {
    const MyNewList2 = this.state.newsList2.map((lists, index) => {
      return (
        <div
        ...
        >
          ...
          <div className="card-body" style={{ height: "10%" }}>
            ...
            <a
              ...
              onClick={() => {
                const ob = this.state.newsList2[index];
                alert(ob.title + " added to favourites");
                Axios.post(
                    "api/updatefav",
                    { headline: ob, user_id: this.state.id },
                    {
                        headers: {
    ...
    return <div className="row rowView">{MyNewList2}</div>;
  }
}
